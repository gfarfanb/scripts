...
rem Usage in script:
rem   call %SCRIPTS_HOME%\.win\require-var <var>

setlocal

set __require_var_tag=%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%-%RANDOM%_%RANDOM%
set __require_var_bat="%SCRIPTS_TEMP_DIR%\require-var-%__require_var_tag%.bat"
...
endlocal & if exist "%__require_var_bat%" del "%__require_var_bat%" & if "%__require_var_flag%"=="1" exit /B 0
